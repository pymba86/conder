...
using Conder.Auth.Middlewares;
using Conder.Auth.Services;
...
namespace Conder.Auth
{
    public static class Extensions
    {
            ...
            Action<JwtBearerOptions> optionsFactory = null)
        {
            ...
            builder.Services.AddSingleton<IAccessTokenService, MemoryAccessTokenService>();
            ...
            if (options.Certificate is {})
            {
                ...
                if (!string.IsNullOrWhiteSpace(options.Certificate.Location))
                {
                    ...
                    var keyType = certificate.HasPrivateKey ? "with private key" : "with public key";

                    Console.WriteLine("Loaded X.509 certificate from location: " +
                                      $"'{options.Certificate.Location}' {keyType}");
                }
