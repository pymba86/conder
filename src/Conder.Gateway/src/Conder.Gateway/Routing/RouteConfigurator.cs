    ...
    internal sealed class RouteConfigurator : IRouteConfigurator
    {
        ...
        private string GetDownstream(Module module, Route route)
        {
            ...
            if (loadBalancerEnabled)
            {
                ...
                {
                    throw new ArgumentException("Load balancer URL cannot be empty",
                        nameof(loadBalancerUrl));
                }
            ...
            if (service is null)
            {
                throw new ArgumentException($"Service for: '{basePath}' was not defined.",
                    nameof(service.Url));
            }
            ...
            {
                throw new ArgumentException($"Service URL for: '{basePath} cannot be empty",
                    nameof(service.Url));
            }
        ...
        private static string SetProtocol(string service) => service.StartsWith("http")
            ? service
            : $"http://{service}";
    }
}