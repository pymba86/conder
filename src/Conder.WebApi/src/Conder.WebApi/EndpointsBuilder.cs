...
namespace Conder.WebApi
{
    public class EndpointsBuilder : IEndpointsBuilder
    {
        ...
        {
            var builder = _routeBuilder.MapGet(path,
                ctx => context?.Invoke(ctx));
        ...
        {
            var builder = _routeBuilder.MapGet(path,
                ctx => BuildQueryContext(ctx, context));
        ...
        public IEndpointsBuilder Get<TRequest, TResult>(string path,
            Func<TRequest, HttpContext, Task> context = null,
            Action<IEndpointConventionBuilder> endpoint = null, bool auth = false,
            string roles = null, params string[] polices) where TRequest : class
        {
            var builder = _routeBuilder.MapGet(path,
                ctx => BuildQueryContext(ctx, context));
            ...
            ApplyAuthRolesAndPolicies(builder, auth, roles, polices);
        ...
        public IEndpointsBuilder Post(string path, Func<HttpContext, Task> context = null,
            ...
            params string[] policies)
        {
            var builder = _routeBuilder.MapPost(path, 
                ctx => context?.Invoke(ctx));
        ...
        {
            var builder = _routeBuilder.MapPost(path, 
                ctx => BuildRequestContext(ctx, context));
        ...
        public IEndpointsBuilder Put(string path, Func<HttpContext, Task> context = null,
            ...
            params string[] policies)
        {
            var builder = _routeBuilder.MapPut(path, 
                ctx => context?.Invoke(ctx));
        ...
        public IEndpointsBuilder Delete(string path, Func<HttpContext, Task> context = null,
            ...
            string roles = null, params string[] policies)
        {
            var builder = _routeBuilder.MapDelete(path, 
                ctx => context?.Invoke(ctx));
                    ...
                    Parameters = new List<WebApiEndpointParameter>
                    {
                        new WebApiEndpointParameter
                        {
                            ...
                            Example = input is null
                                ? null
                                : FormatterServices.GetUninitializedObject(input)
                                    .SetDefaultInstanceProperties()
                        }
                    },
                    Responses = new List<WebApiEndpointResponse>
                    {
                        new WebApiEndpointResponse
                        {
                            ...
                            Example = output is null
                                ? null
                                : FormatterServices.GetUninitializedObject(output)
                                    .SetDefaultInstanceProperties()
                        }
                    }
                });
        ...
        private static void ApplyAuthRolesAndPolicies(IEndpointConventionBuilder builder,
            bool auth, string roles, params string[] policies)
        {
