...
{
    public class KeyValueSecrets : IKeyValueSecrets
    {
        ...
        public async Task<T> GetAsync<T>(string path)
            => JsonConvert.DeserializeObject<T>(
                JsonConvert.SerializeObject(await GetAsync(path)));
        ...
        public async Task<IDictionary<string, object>> GetAsync(string path)
        {
            ...
            {
                throw new VaultException($"Getting Vault secret for path: '{path}' cause an error. " +
