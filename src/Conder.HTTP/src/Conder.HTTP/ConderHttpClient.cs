...
{
    public class ConderHttpClient : IHttpClient
    {
        ...
        public ConderHttpClient(HttpClient client, HttpClientOptions options,
        ...
        private async Task<T> SendAsync<T>(string uri, Method method, HttpContent content = null)
        {
        ...
        private async Task<HttpResult<T>> SendResultAsync<T>(string uri, Method method,
        ...
        private Task<HttpResponseMessage> SendAsync(string uri, Method method, HttpContent content = null)
            => Policy.Handle<Exception>()
                .WaitAndRetryAsync(_options.Retries, r => TimeSpan.FromSeconds(Math.Pow(2, r)))
                .ExecuteAsync(() =>
                {
        ...
        private Task<HttpResponseMessage> GetResponseAsync(string uri, Method method,
            HttpContent content = null)
        {
            switch (method)
            {
        ...
        private StringContent GetJsonPayload(object data)
        {
            if (data is null)
            {
                return null;
